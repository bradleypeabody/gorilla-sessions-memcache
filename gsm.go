...
func (s *MemcacheStore) save(session *sessions.Session) error {
	...
	case StoreMethodSecureCookie:
		...
		err = s.Client.Set(&memcache.Item{Key: key, Value: []byte(encoded), Expiration: int(session.Options.MaxAge)})
		if s.Logging > 0 {
	...
	case StoreMethodGob:
		...
		err := s.Client.Set(&memcache.Item{Key: key, Value: bufbytes, Expiration: int(session.Options.MaxAge)})
		if s.Logging > 0 {
